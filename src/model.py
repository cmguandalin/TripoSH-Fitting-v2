...
Example of usage: 
    ...
    cache_path   = '/path/to/TripoSH-Factory/TripoSH-Fitting/cache/' <---- path to where fiducial kernels are stored
...
class PkBkCalculator:
    ...
    * help()
    ...
    def __init__(self, multipoles, mean_density, redshift, cache_path, fixed_params=['n_s'], rescale_kernels=True, ordering=1):
        ...
        # For fixed cosmology
        self.fixed_cosmology_mode = fixed_params == 'cosmo'
        if self.fixed_cosmology_mode:
            if not os.path.exists(cache_path):
                raise FileNotFoundError(f'Cache directory not found: {cache_path}\n')    
            self.kernel_cache_file = os.path.join(cache_path, 'c000.npy')
            
            if not os.path.exists(self.kernel_cache_file):
                raise FileNotFoundError(f"Precomputed kernels file 'c000.npy' not found in {cache_path}\n")

            # Get precomputed kernels from file
            self.precomputed_kernels = np.load(self.kernel_cache_file, allow_pickle=True).item()
            
            if self.multipoles_pk:
                self.k_pk = self.precomputed_kernels['k']
                self.params_cosmo = self.precomputed_kernels['fiducial_cosmology']
                self._initialise_classPT()
                
            if self.multipoles_bk:
                self.k_bk = self.precomputed_kernels['k']
        
        else:
            print("Running in standard mode: kernels will be obtained from the emulator")
    ...
    '''
        Helper functions
    '''
    def _initialise_classPT(self):
        # Initialise CLASS-PT for power spectrum calculations.
    ...
    def pk_with_fixed_cosmology(self, pars, ell):
        """
        Get power spectrum from CLASS-PT.
        ...
        if ell == '0':
            cs0 = pars.get('c0', 0.0)
            self.Pk_ell = self.classPT.pk_gg_l0(b1, b2, bG2, bGamma3, cs0, 0, ch)
            self.Pk_ell += self.Pstoch
            if fnlequi!=0 or fnlortho!=0:
                tmp = self.classPT.pk_gg_fNL_l0(b1, b2, bG2)
                tmp[np.isnan(tmp)] = 0
                self.Pk_ell += fnlequi * tmp
                self.Pk_ell += fnlortho * self.classPT.pk_gg_fNL_l0_ortho(b1, b2, bG2)
        elif ell == '2':
            cs2 = pars.get('c2pp', 0.0)
            self.Pk_ell = self.classPT.pk_gg_l2(b1, b2, bG2, bGamma3, cs2, ch)
            if fnlequi!=0 or fnlortho!=0:
                self.Pk_ell += fnlequi * self.classPT.pk_gg_fNL_l2(b1, b2, bG2)
                self.Pk_ell += fnlortho * self.classPT.pk_gg_fNL_l2_ortho(b1, b2, bG2)
        elif ell == '4':
            cs4 = pars.get('c4pp', 0.0)
            self.Pk_ell = self.classPT.pk_gg_l4(b1, b2, bG2, bGamma3, cs4, ch)
            if fnlequi!=0 or fnlortho!=0:
                self.Pk_ell += fnlequi * self.classPT.pk_gg_fNL_l4(b1, b2, bG2)
                self.Pk_ell += fnlortho * self.classPT.pk_gg_fNL_l4_ortho(b1, b2, bG2)
        else:
            raise ValueError('Unrecognised multipole.')

        self.interp_function = interp1d(self.k_pk, self.Pk_ell, kind='cubic', fill_value='extrapolate')
        
        return self.interp_function

    def bk_with_fixed_cosmology(self, pars,l1l2L):
        ...
        if Pshot != 0: 
            self.bk_model += ((1+Pshot)/self.mean_density)**2.0
        else:
            self.bk_model += (1/self.mean_density)**2.0

        self.interp_function = interp1d(self.k_bk, self.bk_model, kind='cubic', fill_value='extrapolate')
...
class ModellingFunction:
    ...
    def compute_model_vector(self, theta):
        ...
        If cosmological parameters are fixed, precomputed kernels for a given fiducial cosmology (e.g., c000) are used.
        ...
        if self.fixed_cosmology_mode:
            # Compute predictions for the fiducial cosmology 
            if multipoles_pk:
